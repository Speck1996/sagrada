...
/**ToolCard EglomiseBrush: the effect of this card is that the player can
 * move a die from the window ignoring color restriction,using the playerCommand method. The two states
...
public class EglomiseBrush extends ToolCard {
     ...
     */
    EglomiseBrush(String id, String title, String description){
    ...
    public void playerCommand(String userInput){
        if(this.getState() == ToolCardState.PICKWINDOWDIE)
                ...
                this.placeDie(userInput,false,true);
    ...
    public void activateCard(PlayerInGame player) throws CardNotUsableException{
        this.checkMovableDice(player,false,true);
        this.payCard(player);
        this.setState(ToolCardState.PICKWINDOWDIE,"Remember you can move the die ignoring color restrictions");
    }
